...
//   const [words, setWords] = useState([]);
//   const [topic, setTopic] = useState("");
//   const { fetchSpeech, audioSrc, loadingAudio, isPlaying, togglePlayback } =
//     useSpeak({ text: story });
...
//       if (resp.status === 200) {
//         setStory(resp.data.data);
//         await fetchSpeech(); // Fetch speech only after story is set
//       }
//     } catch (error) {
//       message.error(t("dialogueError"));
//     } finally {
  ...
  const genStory = async (words, topic) => {
    try {
      ...
      if (resp.status === 200) {
        ...
        await fetchSpeech(resp.data.data);
      }
    } catch (error) {
      message.error(t("dialogueError"));
    } finally {
