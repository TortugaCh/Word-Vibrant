...
export async function middleware(req) {
  ...
  const creditRequiredRoutes = ["/stroke-order/practice", "/coloring-page/download"];
  const requiresCredits = creditRequiredRoutes.some(route => pathname.startsWith(route));

  // If the route requires credits, deduct them
  if (requiresCredits) {
    try {
      const creditResponse = await fetch(`${req.nextUrl.origin}/api/auth/checkCredits`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, action: pathname.includes("stroke-order") ? "stroke-order" : "coloring-page" }),
      });
