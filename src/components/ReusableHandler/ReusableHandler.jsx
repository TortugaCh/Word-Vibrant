...
const ReusableHandler = ({
  ...
  const [words, setWords] = useState([]);
  const [values, setValues] = useState({});
  const [confirmed, setConfirmed] = useState(false); // Track confirmation state

  const handleChange = (name, value) => {
    console.log(name, value);
    setValues((prev) => ({ ...prev, [name]: value }));
    setConfirmed(false); // Reset confirmation when any input changes
  };
  ...
  const handleConfirm = () => {
    setConfirmed(true);
    fetchData(); // Fetch data only when the user confirms
  };
    ...
    <div className="max-w-2xl w-full bg-white rounded-3xl shadow-xl p-8 flex flex-col space-y-4 border-2 border-[#F9AF42]">
      ...
      {/* {moduleName === "story" && ( */}
        <button
          ...
          disabled={confirmed}
        >
          {confirmed ? t("optionsConfirmed") : t("confirmOptions")}
        </button>
      {/* )} */}
